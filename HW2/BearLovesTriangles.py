...
def triangle (p1, p2, p3, color): # Рисуем треугольник по трем точкам - p1, p2, p3
    ...
    sortedlist = sorted([p1,p2,p3])
    q1 = sortedlist[0]
    q2 = sortedlist[1]
    q3 = sortedlist[2]
    for x in range(q1[0],q2[0]):
        for y in range(round(min([findEq(q1,q2,x),findEq(q1,q3,x)])),round(max([findEq(q1,q2,x),findEq(q1,q3,x)]))):
            draw.point((x,y),color)
    for x in range(q2[0],q3[0]):
        for y in range(round(min([findEq(q1,q3,x),findEq(q2,q3,x)])),round(max([findEq(q1,q3,x),findEq(q2,q3,x)]))):
            draw.point((x,y),color)                    
    lineP(p1, p2, color)
    lineP(p2, p3, color)
    lineP(p1, p3, color)
